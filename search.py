...
class Node:
    ...
    def __init__(self, state, parent=None, action=None, path_cost=0, heuristica=0):
        """Create a search tree Node, derived from a parent by an action."""
        update(self, state=state, parent=parent, action=action,
               path_cost=path_cost, heuristica=heuristica, depth=0)
        if parent:
            self.depth = parent.depth + 1

    def __repr__(self):
        return "<Node %s - C=%d H=%d>" % (self.state, self.path_cost, self.heuristica)
...
def tree_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            print "Numero de nodos visitados %d" % (count)
            return node
...
def breadth_first_graph_search(problem):
    ...
    return tree_search(problem, FIFOQueue())  # FIFOQueue -> fringe
...
def ramificacion_acotacion_graph_search(problem):
    """return graph_search(problem, lista())"""
    return graph_search(problem, listaOrdenada())
    #return tree_search(problem, listaOrdenada()) #para no usar la lista cerrada se usa tree search.

def ramificacion_subestimacion_graph_search(problem):
    """return graph_search(problem, lista())"""
    return tree_search(problem, listaOrdenadaH())
